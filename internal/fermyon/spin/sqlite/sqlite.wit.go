...
//
//go:nosplit
func Open(database string) (result cm.ErrResult[Connection, Error]) {
...
//go:noescape
func wasmimport_Open(database0 *uint8, database1 uint32, result *cm.ErrResult[Connection, Error])
...
//
//go:nosplit
func Execute(conn Connection, statement string, parameters cm.List[Value]) (result cm.OKResult[QueryResult, Error]) {
	conn0 := (uint32)(conn)
...
//go:noescape
func wasmimport_Execute(conn0 uint32, statement0 *uint8, statement1 uint32, parameters0 *Value, parameters1 uint32, result *cm.OKResult[QueryResult, Error])
